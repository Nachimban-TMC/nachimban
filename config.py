...
REGION_ORDER = ["de", "kr", "eu", "us", "wo"]
REGIONS = {
    # k=섹션 제목(정식명), s=필터 라벨(짧게), en=한글, short=아카이브·메일용 짧은 이름
    # ⚠️ 지역을 추가할 때는 여기만 고치면 된다. 아카이브 라벨(publish),
    #    메일 머리글(notify), 검색 결과 지역명(site) 모두 이 표에서 파생된다.
    "de": {"k": "Germany",        "s": "Germany", "en": "독일",       "short": "독일",  "flag": "🇩🇪", "count": 5},
    "kr": {"k": "Korea",          "s": "Korea",   "en": "한국 · 국내", "short": "한국",  "flag": "🇰🇷", "count": 5},
    "eu": {"k": "European Union",  "s": "EU",      "en": "EU 공통",     "short": "EU",    "flag": "🇪🇺", "count": 3},  # 투자 1건 포함
    "us": {"k": "United States",   "s": "USA",     "en": "미국",        "short": "USA",   "flag": "🇺🇸", "count": 4},  # 주식 2건 포함
    # 위 네 곳에 속하지 않는 지역. 그냥 국제뉴스가 아니라 '해외에 사는
    # 한국인에게 닿는' 것만 — 여행경보·항공·재외국민 안전·국제 제도 변화.
    "wo": {"k": "World",           "s": "World",   "en": "세계",        "short": "세계",  "flag": "🌏", "count": 3},
    # 프랑스는 사용자 요청으로 잠시 제외 (나중에 되살리려면 아래 줄 복구)
    # "fr": {"k": "France", "s": "France", "en": "프랑스", "short": "프랑스", "flag": "🇫🇷", "count": 2},
}

# 지역 코드 → 짧은 이름 / 국기+이름. 위 표에서 파생하므로 따로 관리하지 않는다.
RG_SHORT = {r: m.get("short", m["en"]) for r, m in REGIONS.items()}
RG_FLAG = {r: f"{m.get('flag','')} {m.get('short', m['en'])}".strip() for r, m in REGIONS.items()}
...
REGION_TOPIC_HINTS = {
    ...
    "wo": ("외교부 여행경보 변경, 국제선 운항·공항 파업, 재외국민 안전 공지, "
           "여권·비자 국제 규정 변화, 여러 나라에 걸친 제도 변화. "
           "독일·한국·EU·미국에 해당하는 소식은 그 지역 섹션에 넣고 여기 중복하지 말 것. "
           "국제 정치 해설이나 분쟁 전황 중계는 넣지 말 것 — "
           "'해외에 사는 한국인이 이것 때문에 무엇을 하거나 조심해야 하는가'가 있어야 한다."),
}
